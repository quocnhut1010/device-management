...
export const notificationService = {
  ...
  async getNotifications(isRead?: boolean): Promise<NotificationData[]> {
    try {
      ...
      const response = await api.get<NotificationResponse>(NOTIFICATION_API_BASE, { params })
  ...
  async getUnreadCount(): Promise<number> {
    try {
      const response = await api.get<UnreadCountResponse>(`${NOTIFICATION_API_BASE}/unread-count`)
  ...
  async markAsRead(notificationId: string): Promise<boolean> {
    try {
      const response = await api.put<NotificationMarkReadResponse>(
  ...
  async createNotification(request: CreateNotificationRequest): Promise<boolean> {
    try {
      const response = await api.post<NotificationMarkReadResponse>(
