...
export const getSuggestedReplacementDevices = async (
  oldDeviceId: string
): Promise<SuggestedDeviceDto[]> => {
  const response = await api.get<SuggestedDeviceDto[]>(
    `/Replacement/suggested-devices/${oldDeviceId}`
  )
...
export const getAllAvailableDevices = async (): Promise<SuggestedDeviceDto[]> => {
  const response = await api.get<SuggestedDeviceDto[]>('/Replacement/available-devices')
  return response.data
}
...
export const createReplacement = async (
  ...
  const response = await api.post<ReplacementDto>('/Replacement', createReplacementDto)
  return response.data
}

// Lấy thông tin replacement theo ID
export const getReplacementById = async (id: string): Promise<ReplacementDto> => {
  const response = await api.get<ReplacementDto>(`/Replacement/${id}`)
  return response.data
}

// Lấy lịch sử thay thế thiết bị
export const getReplacementHistory = async (
  ...
  const url = params.toString()
    ? `/Replacement/history?${params.toString()}`
    : '/Replacement/history'
  const response = await api.get<ReplacementDto[]>(url)
  return response.data
}
